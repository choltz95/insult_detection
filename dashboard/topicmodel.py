		...
		for token in text:
			if token in affectivelexicon_dict:
			    affective_senses = affectivelexicon_dict[token]
                            ...
                            affective_senses =  {'anger': 0, 'anticipation': 0, 'disgust': 0, 'fear': 0, 'joy': 0, 'negative': 0, 'positive': 0, 'sadness': 0, 'surprise': 0, 'trust':0 }
...
def remove_stopwords(documents):
    stoplist = loadStopWords('cronscripts/englishstop.txt')
    documents = [re.sub(r'([^\s\w]|_)+', '', document) for document in documents]
...
def topic_model():
    ...
    #record ={"topic_model_json": topic_model_json.replace("'","''"), "topic_model_cyberbullying_json": topic_model_cyberbullying_json.replace("'","''")}
