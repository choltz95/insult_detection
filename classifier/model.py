...
TEST_LABEL_FILE = DIR_PATH + "verification_labels.csv"
...
def ngrams(data, labels, ntrain, mn=1, mx=1, nm=500, binary = False, donorm = False, stopwords = False, verbose = True, analyzer_char = False):
    ...
    with open('vectorizer_' + str(mn) + str(mx) + str(nm),'w') as v:
    ...
    if numFts < X_train.shape[1]:
        ...
        with open('ch2_' + str(mn) + str(mx) + str(nm),'w') as ch:
            pickle.dump(ch2,ch)
...
def run(verbose = True):
    ...
    test_labels_data = readCsv(TEST_LABEL_FILE)
    test_labels = [x[1] for x in test_labels_data]
...
def runClassifiers(X_train, y_train, X_test, y_test = None, verbose = True):
    ...
    for ndx, model in enumerate(models):
        ...
        with open("model" + str(ndx), 'w') as f:
            pickle.dump(model, f)
...
def evaluate(pred,label):
    ...
    rounded_pred = [round(i) for i in pred]
    label = [float(i) for i in label]
    
    for i in range(len(label)):
        if abs(rounded_pred[i] - label[i]) <= 0.01:
            correct += 1
    acc = float(correct)/float(len(label))
    rms = sqrt(metrics.mean_squared_error(pred, label))
    pr,re,fs,sup = metrics.precision_recall_fscore_support(rounded_pred,label) 
    print("rmse error: "+ str(rms))
